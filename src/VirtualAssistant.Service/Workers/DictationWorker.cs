...
public class DictationWorker : BackgroundService, IDictationControl, IDictationService
{
    ...
    private async Task BroadcastDictationEventAsync(DictationEventType eventType, string? text)
    {
        try
        {
            _logger.LogDebug("Broadcasting DictationEvent: {EventType}, text length: {Length}",
            ...
            _logger.LogDebug("Broadcast complete: {EventType}", eventType);
        }
        catch (Exception ex)
        {
            _logger.LogDebug(ex, "Failed to broadcast dictation event {EventType} to SignalR clients", eventType);
        }
    }
