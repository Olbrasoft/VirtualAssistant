...
public sealed class TtsService : IDisposable
{
    ...
    private async Task SpeakDirectAsync(string text, string? source, bool skipCache, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (skipCache)
            {
                // Play directly from memory stream (no cache for dynamic content)
                _logger.LogDebug("Playing from stream (skipCache=true): {Text}", text);
                using var stream = new MemoryStream(audioData);
                await _playbackService.PlayAsync(stream, ".mp3", cancellationToken);
            }
            else
            {
                // Save to cache and play from file (for static/reusable content)
