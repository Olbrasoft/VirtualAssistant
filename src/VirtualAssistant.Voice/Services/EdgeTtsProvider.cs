...
/// <summary>
/// TTS provider using Microsoft Edge TTS API via WebSocket.
/// Pure C# implementation - no external dependencies like Python edge-tts.
/// </summary>
public sealed class EdgeTtsProvider : ITtsProvider
{
    ...
    /// <inheritdoc />
    public bool IsAvailable => true; // TODO: Could check network connectivity or VPN status
    ...
    public async Task<byte[]?> GenerateAudioAsync(string text, VoiceConfig config, CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogError(ex, "WebSocket error generating audio with EdgeTTS");
            return null;
        }
    }
