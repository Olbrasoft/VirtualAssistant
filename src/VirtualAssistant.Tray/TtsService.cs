...
public sealed class TtsService : IDisposable
{
    ...
    public async Task SpeakAsync(string text)
    {
        try
        {
            ...
            {
                Console.WriteLine($"🎤 Mikrofon aktivní - text: {text}");
                return;
            }
            ...
            {
                Console.WriteLine($"Playing from cache: {text}");
                await PlayAudioAsync(cacheFile);
                return;
            }
    ...
    private async Task<byte[]?> GenerateAudioAsync(string text)
    {
