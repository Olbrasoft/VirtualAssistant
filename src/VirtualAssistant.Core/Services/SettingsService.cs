...
/// <summary>
/// JSON file-based settings service for persistent configuration.
/// </summary>
public class SettingsService : ISettingsService
{
    ...
    public SettingsService(ILogger<SettingsService> logger)
    {
        ...
        _logger.LogDebug("Settings file path: {Path}", _settingsPath);
    }
    ...
    public async Task<T?> GetAsync<T>(string key, T? defaultValue = default)
    {
        ...
        try
        {
            ...
            {
                _logger.LogDebug("Settings file doesn't exist, returning default value for key: {Key}", key);
                return defaultValue;
            }
            ...
            if (settings?.TryGetValue(key, out var value) == true)
            {
                ...
                _logger.LogDebug("Retrieved setting {Key} = {Value}", key, result);
                return result;
            }
        ...
        {
            _logger.LogWarning(ex, "Failed to read setting {Key}, returning default value", key);
            return defaultValue;
        }
        finally
        {
    ...
    public async Task SetAsync<T>(string key, T value)
    {
        ...
        {
            _logger.LogError(ex, "Failed to write setting {Key}", key);
            throw;
        }
        finally
        {
